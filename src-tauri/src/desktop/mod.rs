...
static GROUP_WINDOWS: OnceLock<RwLock<Option<(String, Vec<(u32, Character)>)>>> = OnceLock::new();

pub fn get_group_windows() -> &'static RwLock<Option<(String, Vec<(u32, Character)>)>> {
    GROUP_WINDOWS.get_or_init(|| Default::default())
}
        ...
        loop {
            ...
            let group_wins = {
                ...
                groups.last().map(|g| {
                    ...
                    (g.name.to_owned(), characters)
                })
            };

            let Ok(mut g_windows) = get_group_windows().write() else {
                continue;
            };

            *g_windows = group_wins;
